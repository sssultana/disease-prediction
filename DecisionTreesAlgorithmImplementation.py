...
d = pd.read_csv('Pig_Diseases_Training.csv')
X = d[['Skin lesions','Coughing','Sneezing' ,'Diarrhea','Diarrhea with blood','Itching','Dermatitis','Vomiting','Loss of apetite','Higher Body temperature','Reduced milk production','Blood in dung','redness or blue blotching of the skin on ears nose and limbs','nasal and eye discharges','pneumonia','arthritis','Reduced Growth Rate','Scratching']]
Y = d['Disease'].values.tolist()


e = pd.read_csv("Test_dataset.csv")

X_predict = e[['Skin lesions','Coughing','Sneezing' ,'Diarrhea','Diarrhea with blood','Itching','Dermatitis','Vomiting','Loss of apetite','Higher Body temperature','Reduced milk production','Blood in dung','redness or blue blotching of the skin on ears nose and limbs','nasal and eye discharges','pneumonia','arthritis','Reduced Growth Rate','Scratching']]


clf2 = DecisionTreeClassifier()
X = X.to_numpy()
X_predict = X_predict.to_numpy()
clf2.fit(X,range(len(Y)),metric='gini_index')
Y_pred2 = clf2.predict(X_predict)
for i in Y_pred2:
